...
func TestIssueService_GetEditMeta_Fail(t *testing.T) {
	...
	expectedError := fmt.Sprintf(
		"Get %s/rest/api/2/issue/PROJ-9001/editmeta: dial tcp %s: connect: connection refused",
		testServer.URL,
		strings.Replace(testServer.URL, "http://", "", -1),
	)
	if err.Error() != expectedError {
		t.Errorf("Expected to receive error %s, received %v instead", expectedError, err.Error())
	}
}
